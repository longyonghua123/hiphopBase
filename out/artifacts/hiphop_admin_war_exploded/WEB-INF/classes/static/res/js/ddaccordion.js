var ddaccordion = {
	ajaxloadingmsg : '<img src="res/images/loading/loading2.gif" /><br />Loading Content...',
	headergroup : {},
	contentgroup : {},
	...
	expandall : function(b) {
		...
		this.contentgroup[b].filter(":hidden").each(
				function() {
					a.eq(parseInt($(this).attr("contentindex"))).trigger(
							"evt_accordion")
				})
	},
	collapseall : function(b) {
		var a = this.headergroup[b];
		this.contentgroup[b].filter(":visible").each(
				function() {
					a.eq(parseInt($(this).attr("contentindex"))).trigger(
							"evt_accordion")
				})
	...
	toggleone : function(e, c, a) {
		...
		if (typeof a == "undefined" || a == "expand" && b.is(":hidden")
				|| a == "collapse" && b.is(":visible")) {
	...
	ajaxloadcontent : function(f, a, b, e) {
		...
		if (c.status == "none") {
			...
			$.ajax({
				url : c.url,
				error : function(g) {
					d("Error fetching content. Server Response: "
							+ g.responseText)
				},
				success : function(g) {
					g = (g == "") ? " " : g;
	...
	expandit : function(g, a, d, f, c, b) {
		...
		if (e) {
			if (e.status == "none" || e.status == "loading") {
				this.ajaxloadcontent(g, a, d, function() {
		...
		a.slideDown(b ? 0 : d.animatespeed, function() {
			d.onopenclose(g.get(0), parseInt(g.attr("headerindex")), a
					.css("display"), f);
	...
	collapseit : function(d, a, c, b) {
		...
		a.slideUp(c.animatespeed, function() {
			c.onopenclose(d.get(0), parseInt(d.attr("headerindex")), a
					.css("display"), b)
		})
	},
	transformHeader : function(c, a, b) {
		c.addClass((b == "expand") ? a.cssclass.expand : a.cssclass.collapse)
				.removeClass(
						(b == "expand") ? a.cssclass.collapse
								: a.cssclass.expand);
		if (a.htmlsetting.location == "src") {
			c = (c.is("img")) ? c : c.find("img").eq(0);
			c.attr("src", (b == "expand") ? a.htmlsetting.expand
					: a.htmlsetting.collapse)
		} else {
			if (a.htmlsetting.location == "prefix") {
				c.find(".accordprefix").html(
						(b == "expand") ? a.htmlsetting.expand
								: a.htmlsetting.collapse)
			} else {
				if (a.htmlsetting.location == "suffix") {
					c.find(".accordsuffix").html(
							(b == "expand") ? a.htmlsetting.expand
									: a.htmlsetting.collapse)
				}
			}
		...
		jQuery(document)
				.ready(
						function(g) {
							...
							ddaccordion.headergroup[a.headerclass] = g("."
									+ a.headerclass);
							ddaccordion.contentgroup[a.headerclass] = g("."
									+ a.contentclass);
							...
							a.revealtype = a.revealtype.replace(/mouseover/i,
									"mouseenter");
							...
							a.oninit = (typeof a.oninit == "undefined") ? function() {
							}
									: a.oninit;
							a.onopenclose = (typeof a.onopenclose == "undefined") ? function() {
							}
									: a.onopenclose;
							var c = {};
							var f = ddaccordion.urlparamselect(a.headerclass)
									|| ((a.persiststate && b != null) ? b
											: a.defaultexpanded);
							if (typeof f == "string") {
							...
							e.each(function(j) {
								var h = g(this);
								if (/(prefix)|(suffix)/i
										.test(a.htmlsetting.location)
										&& h.html() != "") {
									g('<span class="accordprefix"></span>')
											.prependTo(this);
									g('<span class="accordsuffix"></span>')
											.appendTo(this)
								}
								...
								if (jQuery.inArray(l, f) != -1) {
									ddaccordion.expandit(h, i, a, false, false,
											!a.animatedefault);
									c = {
										$header : h,
										$content : i
									...
									i.hide();
									a.onopenclose(h.get(0), parseInt(h
											.attr("headerindex")), i
											.css("display"), false);
									ddaccordion.transformHeader(h, a,
											"collapse")
								}
							});
							e.bind("evt_accordion", function(j, i) {
								var h = d.eq(parseInt(g(this).attr(
										"headerindex")));
								if (h.css("display") == "none") {
									ddaccordion
											.expandit(g(this), h, a, true, i);
									if (a.collapseprev
											&& c.$header
											&& g(this).get(0) != c.$header
													.get(0)) {
										ddaccordion.collapseit(c.$header,
												c.$content, a, true)
									}
									c = {
										$header : g(this),
									...
									if (!a.onemustopen
											...
											&& g(this).get(0) != c.$header
													.get(0)) {
										ddaccordion.collapseit(g(this), h, a,
												true)
									}
								}
							});
							e.bind(a.revealtype, function() {
								if (a.revealtype == "mouseenter") {
									clearTimeout(a.revealdelay);
									var h = parseInt(g(this)
											.attr("headerindex"));
...
$(function() {
												...
												function() {
													if ($(this).parent().find(
														...
														$(this)
																...
																.find("ul")
																.hide();
														// /* 添加3行代码 tang_wenwei 加号变减号 */
														// /* line1 */
														$(this).parents(".arrowlistmenu").find("ul").find("span").removeClass("spanParentStyle2").addClass("spanParentStyle");
														$(this).parent().find(
																"ul").eq(0)
																.slideDown()
																// /* line2 */
																$(this).parent().find("ul").eq(0).parent().find("span").removeClass("spanParentStyle").addClass("spanParentStyle2");
													} else {
														$(this).parent().find(
																"ul").eq(0)
																.slideUp()
																// /* line3 */
																$(this).parent().find("ul").eq(0).parent().find("span").removeClass("spanParentStyle2").addClass("spanParentStyle");
													}
												})
							}
						} else {
	...
	$(".categoryitems >li").each(
			function() {
				$(this).find("a").eq(0).click(
						function() {
							$(this).parents(".arrowlistmenu").find("li").find(
									"a").removeClass("arrowlistmenuCurrent");
							$(this).addClass("arrowlistmenuCurrent")
						});
				$(this).find("li").find("a").click(
						function() {
							$(this).parents(".arrowlistmenu").find("li").find(
									"a").removeClass("arrowlistmenuCurrent");
							$(this).addClass("arrowlistmenuCurrent")
